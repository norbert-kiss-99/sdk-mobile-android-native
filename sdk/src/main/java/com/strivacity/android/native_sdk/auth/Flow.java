...
public class Flow {
    ...
    public boolean startWorkflow(String challenge, Consumer<String> onLanding) {
        Uri redirectUri = followUntilWithErrorHandling(tenantConfiguration.getEntryEndpoint(challenge), onLanding);
    ...
    private Uri followUntilWithErrorHandling(Uri uri, Consumer<String> landingCallback) {
        ...
        if (response.getResponseCode() == 400) {
            ...
            try {
                ...
                if (!root.has(LANDING_KEY)) {
                    throw new NativeSDKError.OIDCError("OIDC Error", response.getBody());
                }

                landingCallback.accept(root.getString(LANDING_KEY));
                return null;
